...
def main(args: argparse.Namespace) -> int:
    ...
    clab_topo_path = os.path.join(
        ...
        "output", "containerlab_topology", "internet-cisco-iol-l3-lab.clab.yml",
    )
    if os.path.isfile(clab_topo_path):
        print(f"  Topology file : {clab_topo_path}")
        print("  (Generated by generate_clab_topology.py from acl_aerleon/def/)")
    else:
        print(f"\n  WARNING: topology file not found: {clab_topo_path}")
        print("  Run: uv run generate_clab_topology.py")
        input("\n  Generate and deploy the topology, then press Enter to continue...")
