...
import modules.containerlab as containerlab
...
def main(args: argparse.Namespace) -> int:
    ...
    Steps:
        ...
        7  Lab it up          — modules/containerlab.py
    ...
    rep_device = containerlab.get_representative_device(devices)
    topo_path = containerlab.build_topology(args.location, rep_device, output_dir=OUTPUT_DIR)
    print(f"Containerlab topology : {topo_path}")

    clab_host = os.environ.get("CLAB_HOST")
    clab_port = os.environ.get("CLAB_PORT", "22")
    ...
    if clab_missing:
        ...
        input("\nDeploy and validate the lab manually, then press Enter to continue...")
    else:
        clab_port_int = int(clab_port)
        clab_devices = [{"hostname": f"clab-{args.location}", "address": clab_host, "port": clab_port_int}]
